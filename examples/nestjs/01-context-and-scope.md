...
제가 직접 다룬 핵심 범위는 Java 연계 이후의 Push 호출 구조와 서버 발송 구간이며, 최종 수신 측 Flutter 모바일 앱도 직접 개발·구축했다.
...
- Flutter 모바일 앱 직접 개발·구축 및 FCM 연동

기존 업무 서비스 전체는 내가 신규 개발한 범위가 아니다. 반면 Flutter 모바일 앱은 직접 개발·구축했다.
다만 이 저장소는 메시징 서버 경로와 Reliability Evolution을 중심으로 정리하기 때문에, Flutter 앱 내부의 상세 구현은 문서 범위에서 제외한다.
