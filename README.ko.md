...
실무에서 직접 설계·개발·운영한 FCM 알림 발송 경로와, **직접 개발·구축한 Flutter 모바일 앱까지 이어지는 E2E 흐름**을 이직 준비를 하면서 공개 가능한 형태로 다시 정리한 저장소입니다.
...
제가 직접 다룬 중심 범위는 Java 연계 이후의 Push 호출 구조, NestJS 연계 API와 공통 API의 FCM 발송 경로, Redis 상태 관리, FCM 연동, 모니터링과 장애 대응입니다. **최종 수신 측 Flutter 모바일 앱도 제가 직접 개발·구축했습니다.** 다만 기존 업무 서비스 전체를 제가 신규 개발한 것은 아니며, 이 저장소는 메시징 서버 경로를 중심으로 정리했기 때문에 Flutter 앱 내부 상세 구현은 문서 범위에서 제외했습니다.
...
- Flutter 앱 내부 전체 구현을 이 저장소에서 모두 문서화했다는 것
